...
Tags:				responsive, grid, layout, columns, breakpoints
Tested up to:		6.9
...
= 6.9.2 =
* now you can specify anchor ID for every bootstrap tab and then use it in URL hash eg. with JS: bootstrap.Tab.getOrCreateInstance('#your-id').show()
* change blocks apiVersion to version 3

= 6.9.1 =
* added semicolon for inline scripts to fix some minifications
* new transform-origin manipulation in Position controls

= 6.9 =
* tested on WP 6.9
* added Reset button for controls
* added inline-grid to Display options
* added controls for Grid containers and items
* spacing indicators moved from :after to :before and hidden when child selected
* Position is not !important in editor, so it will not jump on (de)select
* rename Experiments settings to JavaScript optimization
* PRO ONLY: new toolbar quick controls for Text Color and Font Size - can be disabled in settings
* PRO ONLY: new Spacing control UI allowing multiple template selection
* PRO ONLY: improvements for copy-paste settings

= 6.8 =
* tested on WP 6.8
* added interactivity support for every block
* new apply_filters( 'bootstrap_css_parent_selector', 'body' ) to change CSS selector specificity for generated styles
* new transform manipulation in Position controls
