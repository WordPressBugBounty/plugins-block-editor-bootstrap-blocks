...
class BootstrapBlocks{
	...
	public function init(){
		...
		add_action( 'wp_head', array( $this, 'calculate_offset_helper' ), PHP_INT_MAX );
	...
	function render_block( $block_content, $block ){
		...
		if( isset( $block['attrs'] ) && isset( $block['attrs']['tempID'] ) && $block['attrs']['tempID'] ){
			if( $this->has_PRO_version ){
				$extraCSSitems = [ 'bsFlexGap', 'bsFlexBasis', 'bsGridTemplateRows', 'bsGridTemplateColumns', 'bsGridTemplateAreas', 'bsGridAutoRows', 'bsGridAutoColumns', 'bsGridAutoFlow', 'bsGridJustifyItems', 'bsGridAlignItems', 'bsGridJustifyContent', 'bsGridAlignContent', 'bsGridGap', 'bsGridColumn', 'bsGridRow', 'bsGridJustifySelf', 'bsGridAlignSelf', 'bsGridOrder', 'bsPosition', 'bsPositionZindex', 'bsPositionTop', 'bsPositionRight', 'bsPositionBottom', 'bsPositionLeft', 'bsPositionTransform', 'bsPositionTransformOrigin', 'bsWidth', 'bsMinWidth', 'bsMaxWidth', 'bsHeight', 'bsMinHeight', 'bsMaxHeight', 'bsAspectRatio' ];
			}else{
				$extraCSSitems = [ 'bsSpacing', 'bsFlexGap', 'bsFlexBasis', 'bsGridTemplateRows', 'bsGridTemplateColumns', 'bsGridTemplateAreas', 'bsGridAutoRows', 'bsGridAutoColumns', 'bsGridAutoFlow', 'bsGridJustifyItems', 'bsGridAlignItems', 'bsGridJustifyContent', 'bsGridAlignContent', 'bsGridGap', 'bsGridColumn', 'bsGridRow', 'bsGridJustifySelf', 'bsGridAlignSelf', 'bsGridOrder', 'bsPosition', 'bsPositionZindex', 'bsPositionTop', 'bsPositionRight', 'bsPositionBottom', 'bsPositionLeft', 'bsPositionTransform', 'bsPositionTransformOrigin', 'bsWidth', 'bsMinWidth', 'bsMaxWidth', 'bsHeight', 'bsMinHeight', 'bsMaxHeight', 'bsAspectRatio' ];
			}
				...
				if( ! isset( $alreadyAdded[ $block['tempID'] ] ) ){
					...
					foreach( $breakpoints as $breakpointSize => $breakpointData ){
						...
						$basicProperties = [
							...
							'bsGridTemplateRows' => [ 'property' => 'grid-template-rows', 'convertToPixels' => false ],
							'bsGridTemplateColumns' => [ 'property' => 'grid-template-columns', 'convertToPixels' => false ],
							'bsGridTemplateAreas' => [ 'property' => 'grid-template-areas', 'convertToPixels' => false ],
							'bsGridAutoRows' => [ 'property' => 'grid-auto-rows', 'convertToPixels' => false ],
							'bsGridAutoColumns' => [ 'property' => 'grid-auto-columns', 'convertToPixels' => false ],
							'bsGridAutoFlow' => [ 'property' => 'grid-auto-flow', 'convertToPixels' => false ],
							'bsGridJustifyItems' => [ 'property' => 'justify-items', 'convertToPixels' => false ],
							'bsGridAlignItems' => [ 'property' => 'align-items', 'convertToPixels' => false ],
							'bsGridJustifyContent' => [ 'property' => 'justify-content', 'convertToPixels' => false ],
							'bsGridAlignContent' => [ 'property' => 'align-content', 'convertToPixels' => false ],
							'bsGridGap' => [ 'property' => 'gap', 'convertToPixels' => true ],
							'bsGridColumn' => [ 'property' => 'grid-column', 'convertToPixels' => false ],
							'bsGridRow' => [ 'property' => 'grid-row', 'convertToPixels' => false ],
							'bsGridJustifySelf' => [ 'property' => 'justify-self', 'convertToPixels' => false ],
							'bsGridAlignSelf' => [ 'property' => 'align-self', 'convertToPixels' => false ],
							'bsGridOrder' => [ 'property' => 'order', 'convertToPixels' => false ],
							...
							'bsPositionTransform' => [ 'property' => 'transform', 'convertToPixels' => false ],
							'bsPositionTransformOrigin' => [ 'property' => 'transform-origin', 'convertToPixels' => false ],
						];
						...
						if( $tempCSS ){
							...
							$parent_selector = apply_filters( 'bootstrap_css_parent_selector', 'body' );
							$breakpointsCSS[ $breakpointSize ] .= esc_html( $parent_selector ) . ' .bs-' . $block['tempID'] . '{' . $tempCSS . '}';
						}
					}
				}
			}
	...
	function load_settings(){
		// default settings
		$this->settings = get_option( 'BootstrapBlocks_settings', [] );
		...
		// default js_optimization
		$this->js_optimization = get_option( 'BootstrapBlocks_experiments', [] );
		$this->js_optimization = array_merge(
			array(
				'js_parts' => 0,
				'js-alert' => 0,
				'js-button' => 0,
			...
			$this->js_optimization
		);

		// default config
		$this->config_options = get_option( 'Bootstrap_config_options', [] );
		...
		$this->config_imports = get_option( 'Bootstrap_config_imports', [] );
	...
	function plugin_admin_tabs( $current = 'general' ){
		$tabs = array( 'general' => __( 'Settings', 'bootstrap' ), 'bootstrap_config' => __( 'Bootstrap configuration', 'bootstrap' ), 'js_optimization' => __( 'JavaScript optimization', 'bootstrap' ) ); ?>
		...
		if( isset( $_POST['settings_nonce'] ) && check_admin_referer( 'save_these_settings', 'settings_nonce' ) ){
			if( $this->tab == 'general' ){
				...
				if( isset( $_POST['reset_settings'] ) ){
					...
					// imports
					$imports = ! isset( $_POST['imports'] ) || ! is_array( $_POST['imports'] ) ? [] : $_POST['imports'];
				...
				$show_update_notice = true;
			}elseif( $this->tab == 'js_optimization' ){
				$js_optimization = ! isset( $_POST['js_optimization'] ) || ! is_array( $_POST['js_optimization'] ) ? [] : $_POST['js_optimization'];
				foreach( $this->js_optimization as $key => $value ){
					$this->js_optimization[ $key ] = isset( $js_optimization[ $key ] ) && intval( $js_optimization[ $key ] ) ? 1 : 0;
				}
				update_option( 'BootstrapBlocks_experiments', $this->js_optimization );
				$show_update_notice = true;
			}
		} ?>
		<div class="wrap">
	...
	function tab_js_optimization(){ ?>
		<table class="form-table">
			<tbody>
				<tr>
					<th colspan="3" style="padding:0">
						<h3 style="margin:0"><?php _e( 'JavaScript optimization', 'bootstrap' ) ?></h3>
					</th>
				</tr>
				<tr>
					<th>
					...
					<td>
						<label for="js_parts">
							<input type="checkbox" name="js_optimization[js_parts]" value="1" id="js_parts" <?php echo checked( $this->js_optimization['js_parts'] ) ?>>
					...
					<td colspan="2">
						...
						<?php foreach( [ 'alert', 'button', 'carousel', 'collapse', 'dropdown', 'modal', 'offcanvas', 'scrollspy', 'tab', 'toast', 'tooltip', 'popover' ] as $js_part ): ?>
							<label for="js-<?php echo $js_part ?>">
								<input type="checkbox" name="js_optimization[js-<?php echo $js_part ?>]" value="1" id="js-<?php echo $js_part ?>" <?php echo checked( $this->js_optimization[ 'js-' . $js_part ] ) ?>>
								<?php echo $js_part ?>
							</label><br>
						<?php endforeach ?>
					</td>
	...
	function wp_enqueue_scripts(){
		...
		if( $this->settings['theme_js'] && ! $this->js_optimization['js_parts'] ){
			wp_enqueue_script( 'bootstrap', plugins_url( 'vendor/twbs/bootstrap/dist/js/bootstrap.bundle.min.js', __FILE__ ), [], '5.2.3', true );
		}
		
		// bo: js_optimization
		if( $this->js_optimization['js_parts'] ){

			$js_parts = [];
			if( $this->js_optimization['js-alert'] ) $js_parts[] = 'alert';
			if( $this->js_optimization['js-button'] ) $js_parts[] = 'button';
			if( $this->js_optimization['js-carousel'] ) $js_parts[] = 'carousel';
			if( $this->js_optimization['js-collapse'] ) $js_parts[] = 'collapse';
			if( $this->js_optimization['js-dropdown'] ) $js_parts[] = 'dropdown';
			if( $this->js_optimization['js-modal'] ) $js_parts[] = 'modal';
			if( $this->js_optimization['js-offcanvas'] ) $js_parts[] = 'offcanvas';
			if( $this->js_optimization['js-scrollspy'] ) $js_parts[] = 'scrollspy';
			if( $this->js_optimization['js-tab'] ) $js_parts[] = 'tab';
			if( $this->js_optimization['js-toast'] ) $js_parts[] = 'toast';
			if( $this->js_optimization['js-tooltip'] ) $js_parts[] = 'tooltip';
			if( $this->js_optimization['js-popover'] ) $js_parts[] = 'popover';
		...
		if( $this->settings['theme_css'] ){
			...
			if( file_exists( $uploads['basedir'] . '/bootstrap/bootstrap.min.css' ) ){
				$snapping_enqueued = true;
				wp_enqueue_style( 'bootstrap', $uploads['baseurl'] . '/bootstrap/bootstrap.min.css', [], '5.2.3' );
			}else{
				wp_enqueue_style( 'bootstrap', plugins_url( 'vendor/twbs/bootstrap/dist/css/bootstrap.min.css', __FILE__ ), [], '5.2.3' );
			}
		}

		if( ! $snapping_enqueued ){
			wp_enqueue_style( 'snapping', plugins_url( 'assets/css/snapping.css', __FILE__ ), [], '5.2.3' );
		}
	}
	...
	function wp_enqueue_scripts_override(){
		...
		wp_enqueue_style( 'bootstrap-blocks', plugins_url( $build_path . '/style-index.css', __FILE__ ), [], filemtime( plugin_dir_path( __FILE__ ) . $build_path . '/style-index.css' ) );
	}
}
